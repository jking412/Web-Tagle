...
func ConnectRedis(address string, username string, password string, db int) {
	once.Do(func() {
		GlobalRedis = newClient(address, username, password, db)
	})
}
...
func newClient(address string, username string, password string, db int) *RedisClient {
	...
	if err := rds.Ping(); err != nil {
		logger.ErrorString("GlobalRedis", "初始化", err.Error())
	}
...
func (rds *RedisClient) Set(key string, value interface{}, expiration time.Duration) bool {
	if err := rds.Client.Set(rds.Context, key, value, expiration).Err(); err != nil {
		logger.ErrorString("GlobalRedis", "Set", err.Error())
		return false
	}
	return true
}
...
func (rds *RedisClient) Get(key string) string {
	...
	if err != nil {
		if err != redis.Nil {
			logger.ErrorString("GlobalRedis", "Get", err.Error())
		}
		return ""
	}
	return result
...
func (rds *RedisClient) Has(key string) bool {
	...
	if err != nil {
		if err != redis.Nil {
			logger.ErrorString("GlobalRedis", "Has", err.Error())
		}
		return false
	}
	return true
...
func (rds *RedisClient) Del(keys ...string) bool {
	if err := rds.Client.Del(rds.Context, keys...).Err(); err != nil {
		logger.ErrorString("GlobalRedis", "Del", err.Error())
		return false
	}
	return true
}
...
func (rds *RedisClient) FlushDB() bool {
	if err := rds.Client.FlushDB(rds.Context).Err(); err != nil {
		logger.ErrorString("GlobalRedis", "FlushDB", err.Error())
		return false
	}
	return true
}
...
func (rds *RedisClient) Increment(parameters ...interface{}) bool {
	...
	case 1:
		...
		if err := rds.Client.Incr(rds.Context, key).Err(); err != nil {
			logger.ErrorString("GlobalRedis", "Increment", err.Error())
			return false
		}
	case 2:
		key := parameters[0].(string)
	...
	default:
		logger.ErrorString("GlobalRedis", "Increment", "参数过多")
		return false
	}
	return true
}
...
func (rds *RedisClient) Decrement(parameters ...interface{}) bool {
	...
	case 1:
		...
		if err := rds.Client.Decr(rds.Context, key).Err(); err != nil {
			logger.ErrorString("GlobalRedis", "Decrement", err.Error())
			return false
		}
	case 2:
		key := parameters[0].(string)
	...
	default:
		logger.ErrorString("GlobalRedis", "Decrement", "参数过多")
		return false
	}
	return true
}
